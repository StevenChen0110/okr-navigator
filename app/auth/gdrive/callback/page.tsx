...
export default function GDriveCallbackPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const redirectUri = `${window.location.origin}/auth/gdrive/callback`;
      ...
      router.replace("/profile?connected=gdrive");
    });
  }, [router]);
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-gray-50 px-4">
        <div className="bg-white rounded-2xl border border-red-200 p-6 max-w-sm w-full text-center space-y-3">
          <p className="text-sm text-red-600">Google Drive 連接失敗：{error}</p>
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-50">
      <span className="text-sm text-gray-400">連接 Google Drive 中，請稍候…</span>
    </div>
  );
}
