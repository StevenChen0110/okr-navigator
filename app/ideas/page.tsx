...
export default function IdeasPage() {
  ...
  async function handleAnalyze() {
    ...
    if (objectives.length === 0) {
      setModalErrorMsg("請先建立至少一個 OKR 目標");
      return;
    }
    setModalErrorMsg("");
    if (isQuickMode) {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6 md:px-6 md:py-10 pb-32 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">想法庫</h1>
          <p className="text-xs text-gray-400 mt-0.5">AI 幫你評估哪個最值得做</p>
        </div>
        <button
      ...
      {pendingItems.length > 0 && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-gray-400">
            點「AI 評估」讓 AI 根據你的 OKR 判斷這件事值不值得做
          </p>
