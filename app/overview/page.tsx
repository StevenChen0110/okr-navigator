...
export default function OverviewPage() {
  ...
  function openModal() { setModalOpen(true); setModalStatus("idle"); }

  function closeModal() {
    setModalOpen(false); setModalStatus("idle");
    setModalTitle(""); setModalWhy(""); setModalOutcome(""); setModalNotes("");
    setModalDetailsOpen(false); setModalAnalysis(null);
    setModalSuggestedLinks([]); setModalSelectedLinkIds(new Set());
    setModalErrorMsg(""); setClarifyQuestion(""); setClarifyAnswer("");
  }

  async function runModalAnalysis(extraNotes?: string) {
    setModalStatus("analyzing"); setModalErrorMsg("");
    try {
      ...
      const result = await callAI<IdeaAnalysis>("analyzeIdea", {
        ...
        ideaNotes: combinedNotes, objectives, progressContext: buildProgressContext(objectives),
      });
      ...
      setModalErrorMsg(e instanceof Error ? e.message : "分析失敗"); setModalStatus("idle");
    }
  }
  ...
  async function handleModalAnalyze() {
    ...
    if (isQuickMode) {
      ...
      try {
        const { shouldClarify, question } = await callAI<{ shouldClarify: boolean; question: string }>("clarifyIdea", { ideaTitle: modalTitle, objectives });
  ...
  async function handleModalSave(ideaStatus: IdeaStatus | undefined, taskStatus: TaskStatus) {
    ...
    const linkedKRs: IdeaKRLink[] = modalSuggestedLinks.filter((l) => modalSelectedLinkIds.has(l.krId)).map((l) => ({ objectiveId: l.objectiveId, krId: l.krId }));
    ...
    const newIdea: Idea = {
      id: uuid(), title: modalTitle, description: descParts.join("\n"),
      analysis: modalAnalysis, createdAt: new Date().toISOString(),
      completed: false, linkedKRs, taskStatus, ideaStatus: ideaStatus ?? "active", quickAnalysis: isQuickMode,
    };
    try {
      await saveIdea(newIdea); setIdeas((prev) => [newIdea, ...prev]); closeModal();
      if (ideaStatus === "deleted") router.push("/tasks?filter=deleted");
      else if (ideaStatus === "shelved") router.push("/tasks?filter=shelved");
    } catch (e) {
      setModalErrorMsg(e instanceof Error ? e.message : "儲存失敗"); setModalStatus("confirm");
    }
  }
    ...
    setObjectives((prevObjs) => {
      ...
      for (const link of linkedLinks) {
        ...
        if (krType === "cumulative") {
          newValue = updatedIdeas.filter((i) => (i.ideaStatus ?? "active") !== "deleted" && i.taskStatus === "done" && (i.linkedKRs ?? []).some((l) => l.krId === link.krId)).length;
          if (kr.targetValue) newValue = Math.min(kr.targetValue, newValue);
        } else if (krType === "milestone") { newValue = status === "done" ? 1 : 0; }
  ...
  function handleDashToggleTodo(ideaId: string, todoId: string) {
    setIdeas((prev) => prev.map((i) => {
      ...
      const todos = (i.todos ?? []).map((t) => t.id === todoId ? { ...t, done: !t.done, doneAt: !t.done ? new Date().toISOString() : undefined } : t);
  ...
  const allKRs = objectives.flatMap((o) => o.keyResults.map((kr) => ({ ...kr, objectiveTitle: o.title, objectiveId: o.id })));
  const oCompletions = objectives.map(calcOCompletion).filter((v): v is number => v !== undefined);
  const avgOCompletion = oCompletions.length > 0 ? Math.round(oCompletions.reduce((a, b) => a + b, 0) / oCompletions.length) : null;
  ...
  const staleKRs = allKRs.filter((kr) => {
    ...
    if (last) { const diff = Math.round((today.getTime() - new Date(last.date).getTime()) / (1000 * 60 * 60 * 24)); return diff >= 7; }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 md:px-6 md:py-10 space-y-6">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="p-5 space-y-4">
              ...
              {(modalStatus === "analyzing" || modalStatus === "saving" || (modalStatus === "clarifying" && !clarifyQuestion)) && (
                <div className="text-center py-10"><div className="text-3xl mb-3 animate-pulse">◎</div>
                  <p className="text-xs text-gray-400">{modalStatus === "saving" ? "儲存中…" : "AI 分析中…"}</p>
                </div>
              )}
              {modalStatus === "clarifying" && clarifyQuestion && (
                <div className="space-y-3">
                  <p className="text-sm text-gray-700 font-medium">{clarifyQuestion}</p>
                  <textarea value={clarifyAnswer} onChange={(e) => setClarifyAnswer(e.target.value)} placeholder="簡單說明即可…" rows={3} autoFocus
              ...
              {modalStatus === "idle" && (
                <div className="space-y-3">
                  <input value={modalTitle} onChange={(e) => setModalTitle(e.target.value)}
                    onKeyDown={(e) => e.key === "Enter" && handleModalAnalyze()} placeholder="用一句話描述你的 Task" autoFocus
              ...
              {modalStatus === "confirm" && modalAnalysis && (
                <div className="space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div><p className="text-sm font-medium text-gray-800">{modalTitle}</p><p className="text-xs text-gray-400 mt-0.5">分析結果</p></div>
                    ...
                    <div key={os.objectiveId} className="bg-gray-50 rounded-lg border border-gray-100 p-4">
                      ...
                      <div className="space-y-1">
                        {os.keyResultScores.map((krs) => (
                          <div key={krs.keyResultId}><ScoreBar score={krs.score} label={krs.keyResultTitle} /><Markdown className="text-xs text-gray-400 mt-0.5">{krs.reasoning}</Markdown></div>
                        ))}
                      </div>
                    </div>
                  ))}
                  {modalAnalysis.risks.length > 0 && (
                    <div className="text-xs text-red-600 bg-red-50 rounded-lg px-3 py-2"><span className="font-medium">風險：</span>{modalAnalysis.risks.join("；")}</div>
                  )}
                    ...
                    <div className="space-y-2">
                      ...
                      {modalSuggestedLinks.map((l) => (
                        <label key={l.krId} className="flex items-center gap-2.5 cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0"><p className="text-xs text-gray-700 truncate">{l.krTitle}</p><p className="text-xs text-gray-400 truncate">{l.objectiveTitle}</p></div>
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => handleModalSave("deleted", "todo")} className="flex-1 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-500 hover:bg-gray-50">放棄</button>
                    <button onClick={() => handleModalSave("shelved", "todo")} className="flex-1 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-600 hover:bg-amber-50 hover:border-amber-200">暫存</button>
                    <button onClick={() => handleModalSave(undefined, "in-progress")} className="flex-1 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">執行</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {krTasksPopup && (() => {
        ...
        const relatedTasks = ideas.filter((i) => (i.ideaStatus ?? "active") === "active" && (i.linkedKRs ?? []).some((l) => l.krId === krTasksPopup.krId));
            ...
            <div className="bg-white rounded-xl w-full max-w-sm shadow-lg overflow-hidden border border-gray-200">
              <div className="px-4 pt-4 pb-3 border-b border-gray-100">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0"><p className="text-xs text-gray-400 mb-0.5">{krTasksPopup.objTitle}</p><p className="text-sm font-semibold text-gray-800 leading-snug">{krTasksPopup.krTitle}</p></div>
              ...
              <div className="divide-y divide-gray-50 max-h-56 overflow-y-auto">
                {relatedTasks.length === 0 ? <p className="px-4 py-6 text-xs text-gray-400 text-center">尚無相關任務</p>
              ...
              <div className="px-4 py-2.5 border-t border-gray-100">
                <Link href={`/tasks?objectiveId=${krTasksPopup.objId}&krId=${krTasksPopup.krId}`} onClick={() => setKrTasksPopup(null)} className="text-xs text-indigo-500 hover:text-indigo-700">＋ 新增任務</Link>
              </div>
            </div>
          </div>
        );
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          {avgOCompletion !== null && <div className="mt-2 h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full rounded-full bg-indigo-400 transition-all" style={{ width: `${avgOCompletion}%`, minWidth: "3px" }} /></div>}
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div className="mt-1 text-xs space-x-1">
            {activeTasks.filter(t => t.taskStatus === "in-progress").length > 0 && <span className="text-amber-500 font-mono">{activeTasks.filter(t => t.taskStatus === "in-progress").length} 進行中</span>}
            {activeTasks.filter(t => t.taskStatus === "done").length > 0 && <span className="text-green-500 font-mono">{activeTasks.filter(t => t.taskStatus === "done").length} 完成</span>}
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            {staleKRs.slice(0, 5).map((kr) => {
              ...
              return (
                <div key={kr.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-xs text-gray-700 truncate">{kr.title}</p><p className="text-xs text-gray-400 truncate">{kr.objectiveTitle}</p></div>
                  <span className="text-xs text-gray-400 shrink-0 font-mono">{daysOld !== null ? `${daysOld}天前` : "從未更新"}</span>
                </div>
              );
            })}
            {staleKRs.length > 5 && <Link href="/okr" className="text-xs text-indigo-500 hover:text-indigo-700 block mt-1">查看全部 {staleKRs.length} 條 →</Link>}
          </div>
        </div>
      )}
      ...
      {objectives.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <button onClick={() => setObjSectionOpen((v) => !v)} className="flex items-center gap-2 flex-1 text-left">
          ...
          {objSectionOpen && <div className="divide-y divide-gray-50">
            {objectives.map((o) => {
              ...
              return (
                <div key={o.id}>
                  <button onClick={() => toggleObjExpand(o.id)} className="w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {completion !== undefined && <span className={`text-xs font-medium font-mono ${getProgressTextColor(completion)}`}>{completion}%</span>}
                        <span className="text-gray-300 text-xs">{isExpanded ? "▲" : "▼"}</span>
                      </div>
                    </div>
                    {completion !== undefined
                      ? <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${getProgressColor(completion)}`} style={{ width: `${completion}%`, minWidth: "3px" }} /></div>
                        ...
                        return (
                          <div key={kr.id} className="pl-2 space-y-1.5">
                            <div className="flex items-center gap-2">
                              ...
                              {krType === "milestone"
                                ? <span className={`text-xs font-medium shrink-0 ${kr.currentValue && kr.currentValue >= 1 ? "text-green-600" : "text-gray-400"}`}>{kr.currentValue && kr.currentValue >= 1 ? "已達成" : "未達成"}</span>
                                : krCompletion !== undefined && <span className={`text-xs font-medium font-mono shrink-0 ${getProgressTextColor(krCompletion)}`}>{krCompletion}%</span>}
                              <button onClick={() => setKrTasksPopup({ krId: kr.id, krTitle: kr.title, objTitle: o.title, objId: o.id })} className="text-xs font-medium px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 hover:bg-gray-200 shrink-0">Tasks</button>
                            </div>
                            <div className="pl-3.5">
                              {krType === "milestone"
                                ? <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${kr.currentValue && kr.currentValue >= 1 ? "bg-green-400" : "bg-gray-200"}`} style={{ width: kr.currentValue && kr.currentValue >= 1 ? "100%" : "0%" }} /></div>
                                : krCompletion !== undefined
                                  ? <div className="flex items-center gap-2"><div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${getProgressColor(krCompletion)}`} style={{ width: `${krCompletion}%`, minWidth: "3px" }} /></div><span className="text-xs text-gray-400 shrink-0 font-mono">{kr.currentValue ?? 0}{kr.unit ? ` ${kr.unit}` : ""} / {kr.targetValue}{kr.unit ? ` ${kr.unit}` : ""}</span></div>
                                  : null}
                            </div>
                          </div>
                        );
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <button onClick={() => setTaskSectionOpen((v) => !v)} className="flex items-center gap-2 flex-1 text-left">
        ...
        {taskSectionOpen && <>
          ...
          {filteredTasks.length === 0
            ? <div className="px-4 py-8 text-center text-xs text-gray-400">{dashTaskFilter === "all" ? "還沒有 Task" : `沒有${TASK_STATUS_LABEL[dashTaskFilter as TaskStatus]}的任務`}</div>
                ...
                return (
                  <div key={idea.id} className={isDone ? "opacity-60" : ""}>
                    <div className="px-4 py-3 flex items-center gap-2">
                      <button onClick={() => toggleTaskExpand(idea.id)} className="flex-1 text-left flex items-center gap-2 min-w-0">
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 bg-gray-50 border-t border-gray-100 space-y-2 pt-3">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {todos.length > 0 && (<><span className="text-xs text-gray-400 font-mono">{doneTodoCount}/{todos.length}</span><div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${getProgressColor(todoPct)}`} style={{ width: `${todoPct}%` }} /></div></>)}
                        </div>
                        <div className="space-y-0.5">
                          {todos.map((todo) => (
                            <div key={todo.id} className="flex items-center gap-2 group rounded-md px-1 py-0.5 hover:bg-white">
                              ...
                              <input ref={(el) => { dashTodoRefs.current[todo.id] = el; }} type="text" defaultValue={todo.title}
                                onBlur={(e) => handleDashUpdateTodoTitle(idea.id, todo.id, e.target.value)}
                                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); handleDashAddTodo(idea.id, todo.id); } if (e.key === "Backspace" && e.currentTarget.value === "") { e.preventDefault(); handleDashDeleteTodo(idea.id, todo.id); } }}
                                className={`flex-1 text-xs bg-transparent border-none outline-none py-0.5 ${todo.done ? "line-through text-gray-400" : "text-gray-700"}`} placeholder="待辦事項" />
                            </div>
                          ))}
                          <button onClick={() => handleDashAddTodo(idea.id)} className="flex items-center gap-2 w-full px-1 py-0.5 text-xs text-gray-400 hover:text-gray-600 rounded-md hover:bg-white">
                            <span className="w-4 h-4 shrink-0 flex items-center justify-center text-gray-300 text-base leading-none">+</span>新增待辦
                          </button>
                        </div>
                      </div>
                    )}
